...
# Initialize database
db = DatabaseManager(db_path=config.database.db_path)
...
if config.speech.eleven_labs_api_key:
    ...
    client = None
    print("WARNING: ELEVEN_LABS_API_KEY not set. Speech features will be disabled.")
...
def chat(
    ...
    try:
        ...
        db.save_conversation(session_id, "user", request.query)

        response_data = process_query(request.query, patient_id=session_id)
        response_text = response_data['messages'][-1].content
        
        db.save_conversation(session_id, "assistant", response_text, agent_name=response_data.get("agent_name"))
...
async def upload_image(
    ...
    try:
        ...
        response_data = process_query(query, patient_id=session_id)
        response_text = response_data['messages'][-1].content
        agent_name = response_data["agent_name"]

        db.save_conversation(session_id, "assistant", response_text, agent_name=agent_name)

        analysis_type = None
        if "BRAIN_TUMOR" in agent_name:
            analysis_type = "brain_tumor"
            result_image = f"/uploads/brain_tumor_output/segmentation_plot.png"
        elif "CHEST_XRAY" in agent_name:
            analysis_type = "chest_xray"
        elif "SKIN_LESION" in agent_name:
            analysis_type = "skin_lesion"
            segmentation_path = os.path.join(SKIN_LESION_OUTPUT, "segmentation_plot.png")
        
        if analysis_type:
            db.save_medical_analysis(
                session_id=session_id,
                analysis_type=analysis_type,
                result_summary=response_text,
                input_image_path=file_path,
                result_details={"agent": agent_name},
            )
...
def validate_medical_output(
    ...
    try:
        db.save_conversation(session_id, "user", f"Validation: {validation_result} {comments or ''}")
        ...
        response_data = process_query(validation_query, patient_id=session_id)

        if latest_analysis:
            db.update_validation(latest_analysis, validation_result, comments)

        db.save_conversation(session_id, "assistant", response_data['messages'][-1].content)
...
def get_patient_profile(session_id: Optional[str] = Cookie(None)):
    """Get the patient profile from long-term memory."""
    if not config.memory.enabled or not session_id:
        return {"profile": None}
    try:
        from agents.memory_agent import PatientMemory
        pm = PatientMemory(config)
        profile = pm.get_patient_profile(session_id)
        return {"profile": profile}
    except Exception as e:
        return {"profile": None, "error": str(e)}

@app.get("/history")
def get_conversation_history(session_id: Optional[str] = Cookie(None)):
    """Get conversation history for the current session."""
    if not session_id:
        return {"history": []}
    try:
        history = db.get_conversation_history(session_id, limit=config.database.conversation_history_limit)
        return {"history": history}
    except Exception as e:
        return {"history": [], "error": str(e)}

@app.get("/analyses")
def get_medical_analyses(session_id: Optional[str] = Cookie(None)):
    """Get medical image analysis records for the current session."""
    if not session_id:
        return {"analyses": []}
    try:
        analyses = db.get_session_analyses(session_id)
...
async def transcribe_audio(audio: UploadFile = File(...)):
    ...
    if not client:
        return JSONResponse(
            status_code=503,
            content={"error": "Speech transcription is disabled. ELEVEN_LABS_API_KEY not set."}
        )
...
async def generate_speech(request: SpeechRequest):
    ...
    if not config.speech.eleven_labs_api_key:
        return JSONResponse(
            status_code=503,
            content={"error": "Speech generation is disabled. ELEVEN_LABS_API_KEY not set."}
        )
    try:
        text = request.text
